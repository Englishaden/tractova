-- Migration 051: get_data_freshness() — add cs_projects block.
--
-- Migration 050 added the cs_projects table (~3,800 NREL Sharing the Sun
-- operating CS projects). This migration extends the freshness RPC so
-- /admin → Data Health surfaces a card for it.
--
-- cs_projects has no cron (NREL's stack blocks server-to-server); the
-- card surfaces last_updated from the seed script + source_release.
--
