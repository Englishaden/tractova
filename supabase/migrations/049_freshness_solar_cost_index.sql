-- Migration 049: get_data_freshness() — add solar_cost_index block.
--
-- Migration 048 added the solar_cost_index table (LBNL Tracking the Sun
-- per-state PV installed-cost percentiles). This migration extends the
-- freshness RPC so /admin → Data Health surfaces a card for it, driven by
-- the most recent successful 'refresh-data:solar_costs' cron run.
--
