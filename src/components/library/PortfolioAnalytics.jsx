...
// ── Portfolio Analytics — the analytics CONTENT of the intelligence drawer ───
// Was WeeklySummaryCard, which carried its OWN navy "Portfolio Intelligence"
// header + collapse chevron — a duplicate of the PortfolioIntelligence drawer it
// renders inside (same title, nested collapsibles, MW/projects repeated ~3×).
// Stripped to header-less, wrapper-less content: Health + Risk row · Tech + Geo
// row · AI insight. The drawer's summary line already owns projects/MW/alerts,
// so the old "Total Capacity" MW tile (a third copy of the MW figure) is gone.
// All scoring / concentration / AI-insight logic is preserved verbatim.
...
export default function PortfolioAnalytics({ projects, stateProgramMap }) {
  ...
  // Risk concentration — % of portfolio MW exposed to a single state/program/tech.
  ...
  // Auto-fire portfolio insight on first load per user-per-day. Manual button
  // still regenerates. Skips if too small (<2) or already cached today.
  useEffect(() => {
    if (!cacheKey) return
    if (aiInsight) return
    if (aiLoading) return
  ...
  // Geographic breakdown (finite-score guard so one bad row doesn't NaN a state).
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {/* Row 1: Health gauge (40%) + Risk Concentration (60%) */}
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-2.5">
        {/* Portfolio Health — animated tier panel (mesh + scan + pulse rings). */}
        {(() => {
          ...
          return (
            <div
              className="relative sm:col-span-2 rounded-xl px-4 py-3 flex flex-col items-center justify-center overflow-hidden"
        ...
        {/* Risk Concentration */}
        <div className="sm:col-span-3 rounded-xl px-4 py-3 bg-gray-50 border border-gray-100">
              ...
              {[
                ...
                <div key={label} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-1.5 rounded-full bg-gray-200 overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${data.pct}%`, background: concColor(data.pct).text }} />
                  </div>
                  <span className="text-[10px] font-bold font-mono tabular-nums w-9 text-right" style={{ color: concColor(data.pct).text }}>{data.pct}%</span>
                </div>
              ))}
              {concentration.state && (
      ...
      {/* Row 2: MW by Technology + Geographic Spread */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
        {/* MW by Technology — donut */}
            ...
            {geoBreakdown.slice(0, 5).map(([state, data]) => (
              <div key={state} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2.5 rounded-full bg-gray-200 overflow-hidden">
                  <div className="h-full rounded-full transition-all" style={{ width: `${totalMW > 0 ? (data.mw / totalMW) * 100 : 0}%`, background: data.avgScore > 65 ? '#0F766E' : data.avgScore >= 40 ? '#D97706' : '#DC2626' }} />
                </div>
      ...
      <div className="rounded-xl border border-gray-100 overflow-hidden" style={{ background: 'linear-gradient(135deg, #F0FDF9, #ECFDF5)' }}>
        {aiInsight ? (
          <div className="px-4 py-4 space-y-3">
            ...
            <button onClick={handleGenerateInsight} className="text-[10px] font-medium text-teal-600 hover:text-teal-800 transition-colors">
              Regenerate
            </button>
          </div>
        ) : aiLoading ? (
          <div className="px-4 py-8 flex items-center justify-center">
            <TractovaLoader size={64} label="Generating Portfolio Insight" sublabel="Analyzing scoring · IX risk · concentration" />
          </div>
        ) : (
          <button onClick={handleGenerateInsight} className="w-full flex items-center justify-center gap-2.5 px-4 py-4 text-xs font-semibold text-teal-800 hover:bg-teal-50/50 transition-colors">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #0F766E, #10B981)' }}>
              <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
            </div>
