...
> **▶ RESUME HERE (F-01 — Stage 1 APPLIED ✅, Stage 2 SHIPPED ✅, Stage 3 awaits Aden prod-eyeball):** Architecture B full-gate + 3-seam coarse preview. **Stage 1** migration 083 APPLIED (is_pro() + 2 `_public` views + DEFINER RPC). **Stage 2 SHIPPED** (pushed, verify-green: 9 smoke + full chain): SPA now reads coarse views for anon. Implementation chose **whole-tab gating** (Analytics+Markets → `PreviewSignupGate` for anon via `effectivePreviewMode`) over per-chart gating — cleaner, no degraded charts, matches "shrink preview to Home". Changes: `programData.js` (getStatePrograms auth-branch, getStateProgramDeltas anon-empty, getDashboardMetricsHistory→snapshots view), `Dashboard.jsx` (tab gate), `smoke.spec.js` (+1 test). Smoke confirmed the `state_programs_public` view is live (083 really applied).
> **⏭ NEXT — ONE Aden action gates the finish:** (A) **Aden prod-eyeballs** (after the Stage-2 deploy lands): logged-out `/preview` Home renders; `/preview?tab=analytics` & `?tab=markets` show the signup gate (not charts/crash); **log in as Pro → `/search` + `/library` still load every panel** (proves the repoint didn't break Pro BEFORE we revoke). Stage 2 doesn't change Pro reads, so this should pass cleanly. (B) **Stage 3 `084_data_tiering_revoke.sql` is WRITTEN & pushed** — uses a name-independent `DO` block that drops EVERY SELECT policy on the 20 gated tables then re-creates gated ones (Pro-only on the 5 calibrated-IP tables; `is_pro() OR is_admin()` on ix_queue_data + policy_impact_events; `authenticated` on the 11 .gov-derived + 2 preview-view base tables). (C) **Aden applies 084 ONLY after (A) passes** → run the anon `count(*)→0` verify block in the file footer, then re-eyeball prod (anon /preview renders; Pro panels load; FREE-authed: county panels load, Pro-IP panels show clean empty states — this also closes an existing free-sees-Pro-data gap). After 084, F-01 is CLOSED. **If a Pro panel breaks at (A), tell Claude before applying 084.**
> **Parallel hardening (no F-01 dependency):** ✅ **F-15** (14 client-facing 500/error responses across api/ genericized — detail kept in console.error/axiomLog only; latent `logAndRespond500` made safe-by-default), ✅ **F-09** (SignIn `humanizeError` collapses 'email not confirmed' + 'invalid login' into one generic message — no enumeration oracle; +rate-limit branch), ✅ **F-14** (vercel.json CSP `img-src 'self' data: https:` → `'self' data:` — only data: bg + same-origin favicons load; **prod-eyeball DevTools for any CSP img violation**), ✅ **F-11** (secrets-manifest.md + CLAUDE.md §7 synced — added HEALTH_CHECK_TOKEN, AXIOM_*, EIA/NREL/CENSUS keys, STRIPE_PRICE_ID pair, LBNL URL, test-user, etc.). All verify-green (9 smoke + full chain), committed+pushed 2026-06-12. **⏭ NEXT: F-06** admin audit trail (hybrid: trigger migration 085 for client-direct writes + app-layer `logAdminAction` in `_refresh-ix-manual`/`_health-staging-post`) — needs a quick RLS/PK probe first (4 tables' write path + revenue_rates/county_intelligence non-`id` PKs are unconfirmed; design mapped in workflow wf_b69afea8-083 output). Then **P3 guardrails** (gitleaks pre-commit + Semgrep CI).
