...
1. **GATED — apply 2 migrations in Supabase, then run the NY IX cutover.** Apply `064_ix_queue_cs_pipeline.sql` (adds `completed_projects`/`completed_mw` — apply promptly or the weekly IX cron fails on missing columns, harmless/logged) AND `065_purge_ssurgo_noncounty_rows.sql` (deletes the 826 SSURGO non-county artifact rows). Then run `node scripts/apply-ny-dg.mjs --apply` (dry-run by default): DELETEs 7 stale NY rows (fabricated seed + 20×-undercounted NYISO-scraper rows), INSERTs 7 real NYSERDA rows. Then browser-verify the NY IX card (single shared Supabase, so couldn't verify pre-cutover).
