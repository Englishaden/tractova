...
## 🟢 Pickup — 2026-05-27 (latest) — Net Billing waves 2+3+4 (12 states sourced) + Geist sans live

**The product now.** Tractova Lens is the 5-pillar signal model (Offtake 25 · IX 25 · Incentives 20 · Site 20 · Policy 10). **Net Billing is now sourced for 12 states** (was 4 at start of day): CA/AZ/UT/ID wave-1 → +AR/IN/LA/MI/NC wave-2 → +OH/KY wave-3 → **+SC wave-4**. HI Smart DER + NY VDER stay deliberately gated (per-interval TOU); VA + MT + IA + NV pulled into NET_METERING_HAIRCUT roadmap (#2); MN deferred (hybrid NM/NB tiered by size, and Xcel Rider QF1 rate not WebFetch-accessible).
...
2. **NET_METERING_HAIRCUT bucket** — applies to states where the Lens currently anchors NM at full retail but the actual structure is reduced: **NV** (75% of retail Tier 4 / NMR-405), **MT** (full retail with annual use-it-or-lose-it surrender), **IA** (full retail with annual avoided-cost cash-out), **VA** (mostly full retail with optional year-end avoided-cost cash-out — smallest haircut). Add `NET_METERING_HAIRCUT[state]` map (or rename to `NET_METERING_EXPORT_RATIO` to mirror NB pattern) — same two-layer signal formula.
