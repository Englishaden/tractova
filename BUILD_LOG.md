...
1. **Extend the cs_pipeline model to NJ + MA** (same pattern as NY). NJ = JCP&L direct `.xlsx` + BPU docket QO21010085. MA = DOER monthly Excel. Both verified GREEN/redistribution-safe. **IL & CA = RED** (Illinois Shines forbids commercial redistribution; CA californiadgstats + PJM Data Miner 2 both forbid scraping) → stay curated/honest. No free national distribution aggregator exists (LBNL=transmission-only; interconnection.fyi=paid).
