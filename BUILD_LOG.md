...
2. **Broaden coverage via HOSTING CAPACITY (the real path to most of the map).** Project-level queue data only exists for ~6 states; hosting-capacity/ICA ArcGIS feeds are open for most big IOUs. See `docs/hosting-capacity-sourcing.md` — verified-open: PG&E (CA), SCE (CA), and the **Exelon ArcGIS org** (`agWTKEK7X5K1Bx7o`) which serves ComEd (IL) + BGE (MD) + PECO/Pepco/Delmarva/ACE (PA/DC/DE/NJ) hosting capacity from ONE org. Send endpoints (or have me research them) → build `hosting_capacity` signalType scrapers (different metric, honest label, score stays curated). Could take real-data coverage from ~3 states to ~15-25.
