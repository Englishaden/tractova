...
2. **GATED — apply migration `066_hosting_capacity_data.sql`**, then the hosting-capacity pipeline goes live. Foundation BUILT 2026-05-23 (`api/scrapers/_refresh-hosting-capacity.js`, wired into refresh-data.js `?source=hosting_capacity` + the admin Refresh button). New `hosting_capacity_data` table = grid HEADROOM per utility (separate from county_geospatial_data — different pillar/metric, no double-count). **BGE (MD) verified clean** (2,601 mile-squares, 41.6% with ≥5MW, server-side aggregation). Until 066 is applied the scraper upsert fails (logged, harmless).
   - **Then: surface it in the IX pillar UI** (new `hosting_capacity` signalType — read `hosting_capacity_data` in programData, add an IX-card branch + glossary entry, score stays curated). Not built yet.
