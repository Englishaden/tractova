...
> **▶ RESUME HERE (F-01 in progress — Stage 1 APPLIED ✅, Stage 2 is next to BUILD):** Data-tiering design done (`security-audit/F01-data-tiering-design.md`, adversarial-reviewed; architecture **B full-gate + a 3-seam coarse preview**). **Stage 1 = migration 083 APPLIED by Aden 2026-06-11** (`is_pro()` helper + `state_programs_public` + `state_programs_snapshots_public` views w/ `security_invoker=off` + `get_dashboard_metrics`→DEFINER). Additive — no access removed yet; anon STILL reads base tables until Stage 3. *(NB: Aden confirmed "applied" but didn't paste the verify-block OUTPUT — on resume, eyeball that the anon counts were right before shipping Stage 2.)*
> **⏭ NEXT, in order:** (2) **Claude builds Stage 2** — the SPA repoint (`src/lib/programData.js`: `getStatePrograms` + `getDashboardMetricsHistory` snapshot-half → `_public` views with an auth-branch [authed reads base for full Pro score, anon reads view]; `getStateProgramDeltas` → anon-empty; gate the 5 IP charts behind `user` [`LmiDivergingLollipop`, `FeasibilityScoreDeltas`, `OperatingCsProjectsDot`/`getCsProjectsAggByState`, `SubscriptionMixChart`/`getCsSubscriptionMixByState`]; fix `IxStalenessAlert`/`ix_queue_data` admin path) → `npm run verify` → push → **confirm on prod logged-out incl. `/preview?tab=analytics` & `?tab=markets` show CTA not data/crash; Pro `/search`+`/library` still load**. (3) Then Claude writes Stage 3 `084_data_tiering_revoke.sql` (REVOKE anon base read; `ix_queue_data` predicate = `is_pro() OR is_admin()`; **probe `pg_policies` for real policy names FIRST** — a missed name = silent leak) → Aden applies only after Stage 2 confirmed live on prod. Full detail: §4/§5 of the design doc.
