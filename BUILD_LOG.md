...
## 🟢 Pickup — §06 polish + migration 063 (FILE) + eyebrow-mono pilot (2026-05-14 evening)

**Three-commit evening, all post-2026-05-13 §05/§06/PDF-intake arc: (1) §06 bucket label rename `edbcccb` — "Earlier on the books · N ▾" → "Earlier events · N ▾" in `LensRegulatoryWatchSection.jsx:277`; Aden flagged prior label as unclear. (2) Migration 063 `e93aa5a` — extends `policy_impact_events.discovered_via` CHECK constraint to include `'pdf_upload'`. FILE only — non-destructive (loosening a CHECK can't reject existing rows). **Awaiting Aden's apply in Supabase SQL editor.** Handler swap in `api/handlers/_lens-policy-classify.js` (currently writes `discovered_via='manual'` + double-stamps `source_type='pdf_upload'` in `discovery_metadata`) is the gated follow-up. (3) §06 eyebrow-mono pilot `43d3f3b` — swapped 3 chip-tag spans (event-type / status / pillar — the small tags at the top of each EventRow) from inline `font-mono text-[8px] uppercase tracking-[0.18em]` to the `.eyebrow-mono` utility (`src/index.css:205`). Visible effect: chips grow 8→9px and 0.18→0.24em on ≥md AND go bold on all screens (utility is font-weight 700; inline was rendering normal-weight by accident of missing `font-bold`). The other 8 mono uses in §06 are intentionally tighter/larger per design vocab hierarchy and stayed inline. Pilot for the broader sweep decision — 244 grep hits across 68 files, but per the §06 sample only ~3 of every 11 are clean candidates.**

**Resume command:** `Continue from BUILD_LOG pickup. §06 "Earlier events" rename shipped; migration 063 (discovered_via 'pdf_upload') shipped as FILE — Aden to apply in Supabase before handler swap follow-up; §06 eyebrow-mono pilot shipped (3 chip-tag spans → utility) — Aden reviewing on prod to decide roll-out vs stay. Remaining backlog: eyebrow-mono sweep beyond §06 (gated on Aden's pilot verdict), Landing tightening (design-subjective), §04/§06 visual-redundancy review (gated on curated events going live), handler swap follow-up to migration 063 (gated on apply), audit-ui parallel-pressure flakes (Dashboard click matrix + Profile gauge — not in verify chain).`
