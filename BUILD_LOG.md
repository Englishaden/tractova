...
1. **Extend hosting-capacity FEEDS** (in `_refresh-hosting-capacity.js`) to PHI (Pepco/Delmarva/ACE → MD/DE/NJ/DC), PECO (PA), PG&E + SCE (CA) — verify each first with `scripts/probe-hosting-capacity.mjs` (GREEN = numeric field + manageable count). The UI/glossary already render any FEED added. **Note:** ComEd (IL) HC is NOT in the open Exelon org, and Ameren IL's feed is unusable (text/multi-value/1.67M cells) — so **IL hosting capacity is not cleanly available**; IL stays curated unless via interconnection.fyi.
