...
> **▶ RESUME HERE (F-01 in progress — Stage 1 BUILT, awaiting Aden apply):** The data-tiering design is done (`security-audit/F01-data-tiering-design.md`, adversarial-reviewed; chose architecture **B full-gate + a 3-seam coarse preview**: 17 tables fully-gated from anon, 2 coarse `_public` views + the dashboard RPC keep the preview alive). **Stage 1 = `supabase/migrations/083_data_tiering_additive.sql` is WRITTEN** (`is_pro()` helper + `state_programs_public` + `state_programs_snapshots_public` views w/ `security_invoker=off` + `get_dashboard_metrics` flipped to DEFINER via ALTER — preserves the live 040 `lastRefreshAt` body). **ADDITIVE — removes no access, zero prod risk.**
> **⏭ NEXT, in order:** (1) **Aden applies migration 083** (Supabase SQL editor) + runs the verify block in the file footer (anon: views return rows, RPC counts non-zero, base reads still work). (2) Once 083 is live, **Claude builds Stage 2** — the SPA repoint (`src/lib/programData.js`: `getStatePrograms`/`getDashboardMetricsHistory`→ `_public` views with an auth-branch; gate 5 IP charts behind `user`; fix `IxStalenessAlert`/`ix_queue_data` admin path) → `npm run verify` → push → **confirm on prod logged-out incl. `/preview?tab=analytics` & `?tab=markets`**. (3) Then Claude writes Stage 3 `084_data_tiering_revoke.sql` (REVOKE anon base read; `ix_queue_data` predicate = `is_pro() OR is_admin()`; **probe `pg_policies` for real policy names first**) → Aden applies only after Stage 2 confirmed live. **Do NOT deploy Stage 2 before 083 is applied** (the SPA reads `state_programs_public`, which 083 creates).
