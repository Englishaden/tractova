...
- Last check left open: eyeball the NY IX card on prod (data + code are live; couldn't pop a browser unprompted). Run a Lens for an NY county → the IX card should show the "CS Interconnection Pipeline · NY-Sun" block (pipeline vs energized) + the "CS Pipeline · Live" badge.

### ⏭ NEXT SESSION — start here
1. **GATED — apply the NJ cutover.** `node scripts/apply-dist-cutover.mjs NJ --apply` (dry-run by default) deletes 2 NJ seed rows (PSE&G 88, JCP&L 45 — fabricated), inserts 1 real JCP&L row (33 CS / 84 MW). Same gate as NY was.
