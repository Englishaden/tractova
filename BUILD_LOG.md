...
**Wave 3 track 2b — FEMA flood = third site constraint (`23489fe`, scoring FOUNDATION only):** Aden's call = treat flood as a third constraint. Shipped the SAFE half: migration **077** (additive flood columns on county_geospatial_data), `computeSiteSubScore` now applies a flat 12-pt penalty when `floodSfhaPct ≥ 20%` (matrix values preserved; unknown→no penalty; clamps ≥0), derivation + `fetchCountyGeospatial` wiring + tests. **No-op until the ingest lands data** (every flood col null today), so no score moves + no user-facing flood copy yet. **⏭ Slice 2 = the FEMA ingest (NOT built — needs Aden):** a data-source FORK — (a) NFHL ArcGIS per-county SFHA-area clipping (matches the `flood_sfha_pct` schema + "NFHL", but GIS-area math is error-prone + unverifiable locally) vs (b) **FEMA NRI county-level flood risk** (clean county data, no geometry math — likely better/safer, but a different metric → would retune the column/threshold). Recommend deciding (b vs a) before building; copy + the ingest cron land with slice 2.
...
**⏭ Wave 3 remaining (Aden to steer):** (a) **Site layers cont.** — FEMA NFHL flood (needs the site-scoring decision above), then slope/DEM · transmission-proximity · PAD-US · NLCD; (b) **IX live-blend runway metric** — needs a design call (cs_pipeline is deliberately NOT blended today). Still open from Wave 1/2: the **2 policy decisions** (HUD-QCT §48e pathway; policy-severity bridge retirement).

**⏰ Waiting on Aden:** (1) **re-run the data refresh** — confirm `geospatial_farmland` is green post-fix + try `?source=geospatial_wetland` (ix_queue 504 was likely transient); (2) **watch the next deploy** — the 10th cron (drop it if Vercel objects); (3) **apply migrations 076 + 077** (both additive/safe, pending); (4) **pick the FEMA ingest data source** (NRI county vs NFHL clipping — see track 2b) before slice 2. Recommend a prod-review pass now — a lot shipped this session that's verify-green but Pro-gated/verify-on-prod.
...
- **077** `county_geospatial_flood.sql` — ADD COLUMN `flood_sfha_pct`/`flood_category`/`flood_sfha_acres`/`flood_last_updated` on `county_geospatial_data` (Wave 3 FEMA flood, third site constraint). Additive + nullable (IF NOT EXISTS) → no existing row/score affected until the flood ingest lands. **⏳ Pending Aden's apply.**
...
- **Pending:** **076** (freshness RPC extension) + **077** (flood columns) — both above, both additive/safe.
