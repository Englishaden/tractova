...
## 🟢 Pickup — 2026-06-12 — SECURITY BUILD COMPLETE (audit + all actionable findings + P3 guardrails shipped) · ⚠ `main` now BRANCH-PROTECTED → ship via branch + PR, not direct push

> **⚠ WORKFLOW CHANGED 2026-06-12:** Aden enabled a GitHub branch-protection ruleset on `main` (PR + passing checks `Lint / unit / build` + `Gitleaks` + `Lockfile tamper scan` required). **`git push origin main` is now REJECTED.** Ship every change via a branch + PR (`git switch -c <slice>` → commit → `npm run verify` → `git push -u origin <branch>` → open PR → merge when green → Vercel deploys prod). See [[feedback-auto-push]] (updated). Alternative if PR friction is too heavy: release-branch promote (`security-audit/03-guardrails-spec.md §2.3b`) keeps direct-push + gates only prod.

> **▶ RESUME HERE (F-01 CLOSED ✅; hardening F-15/09/14/11/06 shipped; NEXT = P3 guardrails):** **F-01 DONE** — migrations 083 (additive: is_pro() + 2 `_public` views + DEFINER RPC) **and 084 (REVOKE) BOTH APPLIED by Aden**; Stage-2 SPA repoint shipped (whole-tab gating: Analytics+Markets → `PreviewSignupGate` for anon; `getStatePrograms` auth-branch; `getStateProgramDeltas` anon-empty; snapshots→`_public` view). Anon now reads ONLY coarse views + DEFINER RPC + news_feed/puc_dockets; synthesized tables are Pro/authed-gated. **Residual (non-blocking, can't be auto-tested — CI has no Pro creds):** next time Aden is logged in as Pro, glance at `/search`+`/library` to confirm panels populate against the live `is_pro()` RLS; if blank, rollback = re-GRANT (per 084 footer) and fix the JWT path. Free-authed now sees clean empty states on Pro-IP panels (also closes an old free-sees-Pro-data gap). OPEN sub-decision: move `scoreEngine.js` server-side (secret) vs keep transparent (`/methodology` already publishes it) — Aden's call, not built.
> **Hardening shipped (2026-06-12, all verify-green 9 smoke + full chain):** ✅ **F-15** (14 client-facing error responses across api/ genericized; latent `logAndRespond500` made safe-by-default), ✅ **F-09** (SignIn enumeration oracle collapsed), ✅ **F-14** (CSP `img-src` → `'self' data:`; 👁 prod DevTools for any img violation), ✅ **F-11** (secrets-manifest + CLAUDE.md §7 synced), ✅ **F-06** (admin audit trail — **migration 085 `trg_admin_audit` trigger ⏭ ADEN APPLIES**; app-layer `logAdminAction` wired LIVE into `_refresh-ix-manual.js` + `_health-staging-post.js` for the 2 service-role admin paths the trigger can't see). Commits `daece05`→F-06 commit.
> **P3 guardrails — CORE SHIPPED ✅ (verify-green, 301 unit + 9 smoke):** CLAUDE.md **§10 secure-coding contract** (14 rules for every future AI diff) + §9 regression-checklist rows; **eslint** `react/no-danger`/`no-eval`/`no-implied-eval`/`no-new-func` (verified zero-hit); **`tests/unit/vercelHeaders.spec.js`** (CSP/header regression gate — freezes script-src/connect-src/img-src/frame-ancestors); **`.gitleaks.toml`** + gitleaks step in `scripts/_git-hooks/pre-commit` + new **`scripts/_git-hooks/pre-push`** (both INERT until `node scripts/install-git-hooks.mjs` re-run); **`.github/workflows/security.yml`** (Semgrep burn-in `continue-on-error` + gitleaks full-history + lockfile-lint; non-blocking, no branch protection) + `.semgrep/tractova-rules.yml` + `.semgrepignore`.
> **P3 ENFORCEMENT SHIPPED ✅ (`b6bfbd6`, verify-green):** lint-api Rules 1–3 (12-fn cap · child_process ban · fetch-allowlist of the 21 verified callsites) + `verify.yml` now runs `lint:js` in CI + `vercel.json` `installCommand: npm ci` (F-20). Actions all green (Security #1 + Verify on e8a94ff).
> **✅ P3 GATE SWEEP DONE:** all 31 entrypoints carry `// GATE: <label>` + lint-api **Rule 4** enforces it (a new ungated route now fails the build). Verify-green. Gates below are the adversarially-verified effective contract (wf_79ef7983-e51) — kept for reference:
> - **webhook-signature:** `api/webhook.js`
> - **cron-secret:** `api/refresh-data.js` · `api/refresh-ix-queue.js` · `api/refresh-substations.js` · `api/refresh-capacity-factors.js` · `api/send-alerts.js` · `api/send-digest.js` · `api/check-staleness.js` · `api/handlers/_health-summary.js` (static HEALTH_CHECK_TOKEN bearer)
> - **admin-bearer:** `api/data-health.js` · `api/handlers/_lens-classify-docket.js` · `_lens-policy-classify.js` · `_health-export.js` · `_health-freshness.js` · `_health-staging-get.js` · `_health-staging-post.js`
> - **pro-bearer:** `api/lens-insight.js` · `api/create-checkout-session.js` · `api/create-portal-session.js` (last two = JWT-gated not tier-gated — note inline) · `api/handlers/_lens-portfolio.js` · `_lens-compare.js` · `_lens-sensitivity.js` · `_lens-scenario-commentary.js` · `_lens-news-summary.js` · `_lens-deal-memo.js` · `_lens-utility-outreach.js` · `_lens-memo-create.js` · `_tract-resolve.js` · `_tractGeoid.js` (pure parser, inherits)
> - **public:** `api/handlers/_lens-memo-view.js` (share-token) · `api/handlers/_market-brief.js`
> Note for the multi-mode dispatchers: `lens-insight.js` ALSO has public (memo-view/market-brief, pre-gate) + admin (classify-docket/policy-classify re-check) paths; `data-health.js` ALSO public (last-refresh) + cron-secret (health-summary). Label each by its PRIMARY gate (above); the per-action nuances live in the handlers' own GATE lines. Run `npm run lint:api` after — Rule 4 must be green.
> **✅ P3 lint-secrets DONE:** added 11 secret patterns (sb_secret_/sb_publishable_/github-pat/PEM/postgres-url/slack-webhook/axiom + env-health/axiom/test-pass/eia/nrel/census), quote-tolerant env-* patterns, `public/` un-skipped (deploys verbatim — F-12), and a **VITE_ manifest gate** (any `VITE_*` in src|api|scripts not in `docs/secrets-manifest.md` fails). Clean across 775 tracked files.
> **✅ F-25 (send-digest) DONE:** `send-digest.js` now authenticates its admin test path via `isAdminFromBearer` (role), email literal removed — every entrypoint is now on the role-based gate. (The broader F-25 item — the `is_admin()`/`_admin-auth.js` legacy email FALLBACK — stays documented-accepted per CLAUDE.md §9, pending the live role-populated check.)
> **⏭ P3 REMAINING (manual / CI-surface only — not local-buildable):** (c) **deploy gating** — MANUAL GitHub branch-protection toggle requiring the Security + Verify checks (spec §2.3b); only Aden can flip it. (d) **osv-scanner** job (pin action tag + `osv-scanner.toml` ignoring the 3 allowlisted advisories) + flip Semgrep off `continue-on-error` once burn-in is triaged on the Actions tab. All P3 *lint* guardrails (lint-api Rules 1–4 + lint-secrets) are LIVE.
> **⏭ P2 hardening Lows (optional / some need an Aden decision):** F-16 PKCE auth flow · F-17 vendor jsdelivr topojson + drop it from CSP connect-src · F-18/F-20 dep cleanup (`npm uninstall shadcn cobe radix-ui`) · F-21/F-22/F-23/F-24 durable security_events log + 5-alert stack · F-26 xlsx byte-cap on the 4 .gov scrapers · the `scoreEngine.js` server-side decision. Roadmap: `security-audit/02-remediation-roadmap.md` P2.
> **⏭ ADEN migrations:** 082 ✅, 083 ✅, 084 ✅ applied; **085 (admin-audit trigger) PENDING** — one SQL-editor session, verify block in the file footer. After 085, F-06 is fully live.
...
- **`src/components/lens/AddressField.jsx` (new)** — on blur, geocodes via the existing proxy and shows inline status: ✓ resolved (normalized address + tract + county + LIC y/n) · ✗ "Couldn't resolve… will run at county level" · or a county-MISMATCH banner with **[Switch to {county}]** (adopts the address's county/state — address-wins) / **[Clear address]**. Resolves on blur only (the free Census geocoder is rate-limited + has no autocomplete endpoint → no per-keystroke typeahead).
- **`src/lib/addressCounty.js` (new)** + 7 tests — `classifyCountyMatch(result, state, county)` → match | mismatch | unknown (fail-open), naming the address's county from the bundled **3,144-row `nmtcLicCounties.js` crosswalk** (offline, no extra calls). Aden's case (Grundy `17063` + Springfield → Sangamon `17167`) now caught.
- **`Search.jsx`** — lifts the settled resolution; the run **reuses** it (no 2nd geocoder call; kills the resolved-once-failed-next flakiness). Pending mismatch → run drops to coherent county-level (never mixes two places).
- **This supersedes planned follow-up #1** (the IncentiveStackPanel "suppress the mismatched line" patch is moot — we prevent the mismatch upstream instead). verify-green: 232 unit / 8 smoke / build / all lints. (Pro-smoke not run locally — missing `TEST_USER_*` creds; Aden prod-reviews on Vercel.)

**⏭ NEXT — small + roadmap:**
1. **HUD QCT row stuck on "Checking" forever (pre-existing display bug).** `getHudQctDda` → `null` (county has no designation) hits `IncentiveStackPanel.jsx`'s `: hudQctDda ? (...) : <LoadingDot/>` branch, so `null` reads as still-loading. **Fix:** mirror the Energy-Community row's `=== null` handling → "No QCT / not designated" for `null` (1-liner).
2. **Phase 3** = slope (USGS 3DEP, verified-live) + protected-land (PAD-US, re-probe the endpoint) site layers — each needs Aden's penalty-weight sign-off. **Phase 4** = IX manual-upload (~5 states) + EIA offtake auto-refresh (⚠ UNIFY with `refresh-substations.js` EIA pipeline, don't duplicate). Near-free: HUD-QCT tract-resolution (`qct_tract_geoids` populated; context-only). Loose end: glossary 'Offtake' long still says "12 high-rate states" (stale vs Wave-3 all-50).

**Deferred (Aden's call):** paid address typeahead (Google Places / Mapbox / Geocodio / Smarty) — true as-you-type suggestions need a paid provider + new secret; revisit if the free resolve-on-blur proves clunky. Census geocoder stays flaky-but-graceful.

**Phase 2 (prior, LIVE):** address→census-tract LIC lookup prod-reviewed ✓ — discrimination live (Grundy IL "Up to 50%" → non-LIC tract drops to "Up to 40%"), graceful fallback + honesty caveats confirmed. Migrations **079 + 080 applied + seeded** (34,992 LIC tracts). Detail in the 3 pickups below.
