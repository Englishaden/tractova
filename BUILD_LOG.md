...
**⏰ 3 things waiting on Aden:** (1) **re-run the data refresh** — confirm `geospatial_farmland` is green post-fix + try `?source=geospatial_wetland` (ix_queue 504 was likely transient); (2) **watch the next deploy** — the 10th cron (drop it if Vercel objects); (3) **apply migration 076** + the **FEMA site-scoring decision** before that layer.
