...
## 🟢 Pickup — §05 path-A validated + §06 Regulatory Watch promoted (2026-05-13 evening)

**Two-shipping-pair this evening: (1) §05 path-A re-enable (commit `933237e`), prod-validated on NY (1351 cs_projects rows), MA (374), and IL (261) without OOM — the 2026-05-11 CsMarketPanel attribution was almost certainly a misread of the recursive-wrapper bug fixed in `e60882f`. Comparable Deals & Benchmarks renders live on Lens results again. (2) §06 Regulatory Watch promoted — new `LensRegulatoryWatchSection.jsx` replaces the un-numbered `MaybeRegulatoryPanel` that previously sat at the bottom of Lens. §06 is a chronological view of `policy_impact_events` (same table §04 uses for pillar scoring) sliced by recency: Pending & Upcoming → Recent (≤90d) → Earlier on the books. Status badges (Pending / Enacted / Partial / Overturned / Expired), event_type tags (Bill / PUC Order / Tariff Change / Rule Filing / Exec Order), pillar chip, FEOC + Safe Harbor flags. Curation-gated PUC dockets fold into §06 as the "◆ Active Proceedings" subsection (hides when no `puc_dockets` row exists for the state). `MaybeRegulatoryPanel` deleted from `MaybeLensPanels.jsx` per clean-code discipline. Glossary entries added for "Regulatory Watch" and "Active Proceedings."**

**Resume command:** `Continue from BUILD_LOG pickup. §05 path-A live + prod-validated (NY/MA/IL clean); §06 Regulatory Watch shipped reframing the PUC-dockets section as a chronological policy-events feed. Next planned slice: PDF/text-paste intake for policy_impact_events — admin pastes raw PDF text (PUC press release, state-legislature PDF, utility-commission order) → AI extractor returns structured draft policy_impact_events row → admin reviews + edits + publishes → surfaces in §06. Lowers the curation barrier so the chronological feed has steady supply. Other open backlog: long-tail eyebrow-mono sweep (admin-only tight slice if at all), Landing tightening (design-subjective), audit-ui parallel-pressure flakes (Dashboard click matrix + Profile gauge — not in verify chain).`
...
**Why this matters strategically:** Aden's curation labor is asymmetric — sourcing policy articles is leverageable (RSS, news aggregators, Utility Dive); manual PUC docket transcription per-state is brutal (every state PUC has its own archaic CMS, no API). §06 reuses the `policy_impact_events` table whose `discovered_via` enum already supports `news_ai_suggest` and `docket_ai_suggest` — so the curation funnel matches the labor reality. Next slice (PDF-text paste intake) widens the funnel further: paste raw PDF text → AI extractor → structured draft → admin review → published.
...
- **PDF intake is the obvious next slice.** Admin tab: textarea for pasted PDF text → POST to a new (or extended) API endpoint → Anthropic tool-use returns structured `policy_impact_events` draft → admin reviews + edits + publishes. Lowers the per-state curation cost dramatically. Will need design discussion on: (a) whether to extend `api/lens-insight.js` or add a new endpoint (Vercel Hobby 12-function cap is still relevant), (b) whether to add `discovered_via='pdf_paste'` enum value via migration FILE, (c) whether the admin UI lives in the existing Policy Impact tab or as a new tab.

**Verification (close of session):**
- `npm run lint:api / lint:secrets / lint:locs` — clean
- `npm run test:unit` — 129/129 green
- `npm run build` — clean (3.44s)
- `npm run test:smoke` — 7/7 green
- §05 re-enable commit (`933237e`) shipped + prod-validated separately earlier this evening.
