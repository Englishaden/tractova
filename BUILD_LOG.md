...
- **Branch:** `main` · last commit `dfa36c3` axiom: await axiomLog in error paths so fetch completes before serverless tear-down. **Axiom logging confirmed working in production** as of 2026-05-07 — first event landed via webhook signature failure path; service=tractova-api, route=api/webhook, region=iad1. Root cause was Vercel-serverless tearing down the function before fire-and-forget fetch resolved; fixed by making axiomLog awaitable + adding await at all 10 callsites. AXIOM_TOKEN row added to docs/SECURITY_ROTATION_LOG.md (annual cadence, next due 2027-05-07). **Aden-side queue:** close 6 Dependabot major-bump PRs in GitHub UI (now blocked from re-opening by .github/dependabot.yml ignore rule).
