...
> **⏭ P3 REMAINING (manual / CI-surface / small):** (c) **deploy gating** — MANUAL GitHub branch-protection toggle requiring the Security + Verify checks (spec §2.3b); only Aden can flip it. (d) **osv-scanner** job (pin action tag + `osv-scanner.toml` ignoring the 3 allowlisted advisories) + flip Semgrep off `continue-on-error` once burn-in is triaged on the Actions tab. (e) **F-25** drop `send-digest.js`'s hardcoded ADMIN_EMAIL literal → `isAdminFromBearer`/role (the GATE comment there flags it; last entrypoint on the legacy email check). All P3 *lint* guardrails (lint-api Rules 1–4 + lint-secrets) are now LIVE.
