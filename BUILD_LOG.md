...
> **⏭ P3 DEFERRED (need prereq sweeps / GitHub surface — do as a focused pass, NOT inert):** (a) **lint-api.mjs security rules** (fn-cap/child_process/fetch-allowlist/GATE) — Rule 4 needs a one-time `// GATE:` comment sweep across the 12 routes + handlers FIRST, else lint:api goes red; (b) **lint-secrets.mjs** new patterns + `public/` unskip + VITE_ manifest gate — verify completeness first; (c) **verify.yml** edits (add lint:js to CI) + **deploy gating** (branch protection OR release-branch promote — MANUAL GitHub/Vercel steps, spec §2.3b); (d) **osv-scanner** job (pin action + osv-scanner.toml ignoring allowlisted advisories); (e) vercel.json `installCommand: npm ci` (F-20). Aden must glance at the **Actions tab** after this push to confirm security.yml runs (gitleaks/lockfile green, Semgrep burns in).
