...
1. **Hosting-capacity feeds — PECO (PA) added 2026-05-23** (`hosting_capacity_data` now MD/BGE + PA/PECO, both live). The clean-feed pattern: only utilities that PRE-AGGREGATE to a coarse grid work for server-side stats. **Verified NOT usable** (don't re-investigate): PHI Pepco/Delmarva/ACE (`Feeder_Large_Gen_HC` is kW + capped at 3MW, murky), PG&E (LineDetail = 1.3M segments, kW), Ameren IL (text/multi-value, 1.67M), SCE (behind Open-Data hub search, FeatureServer unresolved). So clean hosting-capacity ≈ MD + PA only; broader needs per-utility coarse-grid feeds or a tile-pull approach for the granular ones. ComEd (IL) HC not in the open Exelon org → IL stays curated unless via interconnection.fyi.
