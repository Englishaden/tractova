...
> **⏭ NEXT: P3 guardrails** (prevention layer, spec in `security-audit/03-guardrails-spec.md`): gitleaks pre-commit hook + `.gitleaks.toml`, a `.github/workflows/security.yml` (Semgrep p/owasp-top-ten + custom Tractova rules, npm-audit via existing `audit-check.mjs`, gitleaks action), lint-api/lint-secrets rule additions, and the CLAUDE.md §10 secure-coding contract. Then the remaining roadmap Lows (`security-audit/02-remediation-roadmap.md` P2 tail). **⏭ ADEN: apply migrations 084 done ✅; 085 pending** (one SQL-editor session; verify block in each file footer).
