...
> **⏭ P3 — TURNKEY NEXT: GATE-comment sweep + lint-api Rule 4.** Add `// GATE: <label>` (in the first 2000 chars) to all 31 entrypoints, THEN append Rule 4 to `scripts/lint-api.mjs` (the `// GATE:` presence check from guardrails spec §1.3; `needsGate` = top-level `api/*.js` + `api/handlers/_*.js`). Gates are ADVERSARIALLY VERIFIED (workflow wf_79ef7983-e51) — use this table verbatim, no re-discovery:
...
> **⏭ P3 ALSO DEFERRED:** (b) **lint-secrets.mjs** edits (new secret patterns + `public/` unskip + VITE_ manifest gate) — make edits then RUN `npm run lint:secrets` to confirm no false-positive on tracked `public/` files; all 5 src VITE_ vars are already in the manifest. (c) **deploy gating** (branch protection OR release-branch promote — MANUAL GitHub/Vercel, spec §2.3b). (d) **osv-scanner** job (pin action tag + osv-scanner.toml ignoring the 3 allowlisted advisories) + flip Semgrep off `continue-on-error` after burn-in. (e) **F-25** drop the `send-digest.js` hardcoded ADMIN_EMAIL literal → `is_admin()`/role (last entrypoint on the legacy email check).
