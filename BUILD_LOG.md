...
## 🟢 Pickup — 2026-06-12 — SECURITY SESSION CLOSED OUT ✅ · audit → every actionable finding fixed → P3 guardrails → branch protection · `main` is now PR-gated
...
**The queued security analysis was executed end-to-end.** Full-spectrum multi-agent audit → `security-audit/` (5 docs: `00-threat-model`, `01-findings`, `02-remediation-roadmap`, `03-guardrails-spec`, `04-unverified-checklist`; + `F01-data-tiering-design`). 27 findings (0 Critical · 1 High · 6 Medium · 20 Low); 17 prior controls intact; 7 refuted. **Every actionable finding fixed + shipped, verify-green throughout. Migrations 082 / 083 / 084 / 085 ALL APPLIED.**

**Findings remediated (commits on `main` through `873312f`):**
- **F-01 (the lone High) — CLOSED.** Data-tiering: 083 (`is_pro()` + coarse `state_programs(_snapshots)_public` views + DEFINER `get_dashboard_metrics`) + 084 (REVOKE anon base read on the 20 synthesized tables) applied; SPA repointed (anon reads only coarse views + RPC + news_feed/puc_dockets; synthesized IP is Pro/authed-gated). Whole-tab gating: Analytics+Markets → `PreviewSignupGate` for anon.
- **Mediums:** F-02 (webhook-ledger EXECUTE revoke, 082) · F-03/F-04/F-05 (lens-insight prompt-input caps / cache-poisoning key fix / fail-closed limiter + daily cap) · F-06 (admin audit trail — 085 `trg_admin_audit` trigger + app-layer `logAdminAction` in `_refresh-ix-manual` + `_health-staging-post`).
- **Lows:** F-08 (082) · F-09 (SignIn enumeration) · F-11 (env-var manifest sync) · F-13 (axiom-probe redact) · F-14 (CSP img-src) · F-15 (14 error-response leaks) · F-25 (send-digest role gate) · F-44 (export formula sanitization).

**P3 guardrails — prevention layer, all LIVE:** CLAUDE.md **§10 secure-coding contract** + §9 rows · **lint-api Rules 1–4** (12-fn cap · `child_process` ban · fetch-allowlist · `// GATE:` on all 31 entrypoints, adversarially verified `wf_79ef7983-e51`) · **lint-secrets** (+11 patterns, `public/` scanned, VITE_ manifest gate) · ESLint `no-danger`/`no-eval` · `tests/unit/vercelHeaders.spec.js` (CSP regression gate) · `.github/workflows/security.yml` (Semgrep burn-in + gitleaks full-history + lockfile-lint; Actions green) · `.gitleaks.toml` + pre-commit/pre-push hooks · `verify.yml` lint:js + `vercel.json npm ci` · **GitHub branch-protection ruleset on `main`** (PR + checks required; validated by PR #24).

**⏭ OPTIONAL / FUTURE — none blocking; Aden's call on priority:**
- **One residual eyeball:** logged in as Pro, confirm `/search`+`/library` panels populate against the live `is_pro()` RLS (CI has no Pro creds; rollback if blank = re-GRANT per the 084 footer, then fix the JWT path).
- **P2 hardening Lows** (`security-audit/02-remediation-roadmap.md` P2): F-16 PKCE · F-17 vendor jsdelivr topojson → drop from CSP `connect-src` · F-18 dep cleanup (`npm uninstall shadcn cobe radix-ui`) · **F-21–24 durable `security_events` log + 5-alert stack** ← the monitoring/threat-detection layer Aden originally wanted; best next project · F-26 xlsx byte-cap on the 4 .gov scrapers · the `scoreEngine.js` server-side-vs-transparent decision (`/methodology` already publishes the formula).
- **CI polish:** flip Semgrep off `continue-on-error` after burn-in triage · add osv-scanner (pin action tag + `osv-scanner.toml` ignoring the 3 allowlisted advisories).
- Decision still open: `scoreEngine.js` ships client-side (minified, not secret); move server-side vs keep transparent — not built.
