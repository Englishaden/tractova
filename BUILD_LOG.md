...
**Real live IX coverage = 11 CS states** (NY, NJ via queue · MD, MA, CA, CT, ME, RI, CO, MN, MI via hosting capacity) + PA bonus. Everything else curated · zero fabricated "live".
- **Double-check 2026-05-23 (Aden asked "you sure?"):** re-verified the blocked states + found **MI** = Indiana Michigan Power / AEP (`PROD_MI_HC_GRID`, `MAX_HCLOAD` kW, 94k sections, max 7.5MW) — added. Confirmed truly RED: ComEd IL (HC service exists but **token-gated**), Avista WA (only field caps 50kW), Dominion VA (only `LIMIT_VAL` bucketed codes — not real MW, excluded), FPL/HECO/PacifiCorp/PGE/PSE/PNM/We Energies (junk search hits, no real feed). DTE MI skipped (caps 1MW; AEP I&M covers MI better).
1. **Hosting-capacity feeds — 15 utilities / 9 CS states + PA (2026-05-23).** `hosting_capacity_data` live: **CA** (SCE + PG&E), **NY** (Con Edison, National Grid, O&R, Central Hudson, NYSEG/RG&E), **MA** (National Grid), **MD** (BGE), **PA** (PECO), **CT** (Eversource), **ME** (Central Maine Power / Avangrid), **RI** (Rhode Island Energy), **CO + MN** (Xcel — STRING fields via pull+parse). Scraper handles: server-side count+buckets+max (outStatistics→orderBy fallback), unit-aware kW/MW, 60s timeout for slow self-hosted MapServers (Eversource), and a **pull+parse path** for STRING capacity fields. Metric = `sites_by_threshold` (0.15→50MW buckets, every size) shown at the bucket nearest the developer's MW; max is null-safe (CT's MapServer won't return max — UI hides it).
   - **Verified NOT usable** (don't re-investigate): Ameren IL (text/multi-value 1.67M), ComEd IL (no open), Avista WA (GEN_MAX_KW caps 100kW — wrong scale), Dominion VA (bucketed ints), DTE MI (caps 1MW), PHI MD (capped 3MW), UI CT (only symbolcolor), Versant ME (SaaS), PSE WA / HECO HI (gated/address-tool), PacifiCorp+PGE OR / FPL+Duke FL / APS / NV / GA / PNM / We / Alliant / Consumers / Appalachian (no open feed).
   - **Still-blocked CS states (no scraper possible, re-verified 2026-05-23):** IL (ComEd token-gated, Ameren text-1.67M), VA (Dominion bucketed codes only, Appalachian none), FL, HI, WI, NM, OR, WA. Need manual-upload or paid.
2. **Manual-upload ingest pipeline for the gated *queue* sources** (e.g. NJ PSE&G/ACE BPU-docket PDFs, MA Eversource). Aden downloads monthly → Supabase Storage → ingest parser. Not yet built.
3. **interconnection.fyi** (paid, quote-only, 16 states) — the only path to IL + consistent queue data. Aden investigating.
4. **Browser-verify** the IX cards render on prod for the live states (couldn't pop a browser unprompted).
