...
> **⏭ NEXT, in order:** (A) **Aden prod-eyeballs** (after the Stage-2 deploy lands): logged-out `/preview` Home renders; `/preview?tab=analytics` & `?tab=markets` show the signup gate (not charts/crash); **log in as Pro → `/search` + `/library` still load every panel** (proves the repoint didn't break Pro BEFORE we revoke). (B) Then **Claude writes Stage 3 `084_data_tiering_revoke.sql`** — REVOKE anon base read; `ix_queue_data` predicate = `is_pro() OR is_admin()`; **probe `pg_policies` for real policy names FIRST** (a missed name = silent leak). Confirmed-safe names already in design doc §3. (C) Aden applies 084 only after (A) passes → run the anon `count(*)→0` verify block. **Free-authed UX:** post-084, free logged-in users lose `is_pro()`-gated panels in /search+/library (closes an existing free-sees-Pro-data gap) — eyeball graceful empty states.
