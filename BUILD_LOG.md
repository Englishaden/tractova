...
- ⚠️ **Hosting capacity is COLLECTED but NOT YET SURFACED in the product** — nothing in the Lens reads `hosting_capacity_data` yet. The IX-card UI branch + glossary + (optional) score wiring is the missing consumption layer.
- Last check left open: eyeball the NY + NJ IX cards on prod (data + code live; couldn't pop a browser unprompted). NY → "CS Interconnection Pipeline · NY-Sun" block + "CS Pipeline · Live" badge; NJ → "· JCP&L territory" block (pipeline only, no energized).

### ⏭ NEXT SESSION — start here
1. **Honesty cleanup — demote the 6 remaining fabricated-seed states (CO, IL, MA, MD, ME, MN).** After NY+NJ cut over to real data, these 6 still have seed/empty-ISO-scraper rows in `ix_queue_data` that (a) show "IX · Live", (b) no longer refresh (ISO scrapers deleted), (c) blend fabricated study-months into the score. They should be demoted to curated (delete the rows → IX score reverts to pure curated `ixDifficulty`, no fake "live"). MA/MN could later get real data via manual-upload; CO/IL/MD have no clean feed. GATED (prod DELETE + 6 states' IX scores shift to baseline). Aden flagged the NJ cleanup; this is the same cleanup for the rest.
2. **Surface hosting capacity IN THE PRODUCT (the missing consumption layer).** Migration 066 is applied + BGE is collecting weekly, but nothing reads `hosting_capacity_data` yet. Build: `programData.getHostingCapacity(state)`, an IX-card branch showing "X% of grid with ≥5MW headroom · avg N MW" (honest grid-headroom label, score stays curated), glossary entry. Until this lands, the data is invisible to users.
   - **Then: extend FEEDS** to PHI (Pepco/Delmarva/ACE → MD/DE/NJ/DC), PECO (PA), PG&E + SCE (CA) — verify each first with `scripts/probe-hosting-capacity.mjs` (GREEN = numeric field + manageable count). **Note:** ComEd (IL) HC is NOT in the open Exelon org, and Ameren IL's feed is unusable (text/multi-value/1.67M cells) — so **IL hosting capacity is not cleanly available**; IL stays curated unless found elsewhere or via interconnection.fyi.
3. **Manual-upload ingest pipeline for the gated queues (MA, MN, CO, MD).** These can't be auto-fetched (MA = JS/Salesforce-gated; MN/CO = Xcel Salesforce portal; MD = PDF). Pattern: Aden downloads the file monthly → Supabase Storage bucket → an ingest parser. Not yet built.
4. **RED queue sources stay curated:** IL Illinois Shines + CA californiadgstats + PJM Data Miner 2 all forbid redistribution. Paid aggregator interconnection.fyi (16 states) is quote-only/closed-license. Aden's call whether internal-score-only use of a no-redistribute source is acceptable.
