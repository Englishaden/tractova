...
### ✅ APPLIED + VERIFIED 2026-05-22 (was gated — now live)
- Migrations `064` + `065` applied by Aden. **NY IX cutover ran** (`apply-ny-dg.mjs --apply`): 7 stale NY rows deleted, 7 real NYSERDA rows in (`ix_queue_data` data_source now `{seed:9, scraper:3, nyserda_cdg:7}`; ConEd 580 pipeline/636 energized, NatGrid 240, NYSEG 138…). **county_geospatial_data 3,969 → 3,143** (= canonical count; 0 extras, 0 malformed). Both verified in live DB via the probe scripts.
- **NJ cutover applied 2026-05-23** (`apply-dist-cutover.mjs NJ --apply`): deleted 2 fabricated NJ seed rows (PSE&G 88, JCP&L 45), inserted 1 real JCP&L row (33 CS / 84 MW). `ix_queue_data` now `{seed:7, scraper:3, nyserda_cdg:7, nj_ic_queue:1}` = 18 rows. Verified live.
- **Migration 066 applied + hosting-capacity LIVE 2026-05-23.** `hosting_capacity_data` populated (BGE/MD: 1083/2601 mile-squares ≥5MW, 41.6%, avg 7.8MW). Wired into the weekly cron (`refresh-data?source=all`, Sun 7am UTC) + admin Refresh. NOTE: we store the per-utility AGGREGATE (total/with-capacity/%/avg/max), recomputed server-side each refresh — NOT the raw 2,601 cells.
- **Hosting capacity SURFACED in the Lens 2026-05-23** (`9c5acf7`): `getHostingCapacity()` → "Distribution Hosting Capacity" block in the IX detail card (% grid open ≥5MW + avg/max per utility) + §04 summary caption + glossary "Hosting Capacity · Live". Honest grid-headroom framing; score stays curated.
- **6 fabricated-seed states DEMOTED 2026-05-23** (`demote-seed-states.mjs --apply`): deleted 10 seed/empty-scraper rows (CO/IL/MA/MD/ME/MN). **ix_queue_data now 8 rows = `{nyserda_cdg:7, nj_ic_queue:1}`** (only NY+NJ). Those 6 states now show IX = curated baseline (no fake "live"); MD surfaces real BGE hosting capacity. **IX pillar is now fully honest: zero fabricated "live" data.**
- Last check left open: eyeball the NY + NJ IX cards + the MD hosting-capacity block on prod (data + code live; couldn't pop a browser unprompted).

### ⏭ NEXT SESSION — start here
**IX pillar is now fully honest (NY+NJ real CS pipeline · MD real hosting capacity · everything else curated · zero fake "live").** Remaining = broaden real coverage:
1. **Hosting-capacity feeds — expanded to 9 utilities / 6 states 2026-05-23.** `hosting_capacity_data` live for: **CA** (SCE 121k circuit-segments ≥5MW), **MA** (National Grid), **MD** (BGE), **PA** (PECO), **NY** (National Grid + Con Edison + Orange & Rockland + Central Hudson + NYSEG/RG&E). All numeric, server-side aggregated (corrected earlier error: server-side `outStatistics` works on ANY row count — only TEXT fields like Ameren are the real blocker). Scraper is unit-aware (kW↔MW). **Metric reframed to be granularity-robust:** lead with "# sites that can host ≥5MW" + "best MW", per-utility only (no cross-granularity rollup — feeders vs nodal-segments vs quads aren't comparable). **Verified NOT usable** (don't re-investigate): DTE-MI (caps at 1MW), PHI (kW capped 3MW murky), Dominion (bucketed ints), Xcel CO/MN (STRING fields), Ameren IL (text/multi-value); Eversource/Duke/APS/NV/SDG&E/GA/PNM/We/Alliant/Consumers (gated or none). PG&E (CA) works server-side but SCE already covers CA. ComEd (IL) HC not openly published → IL stays curated.
   - **Size-complete metric (067 applied + populated 2026-05-23):** capture EVERY datapoint, no small-project exclusion. `sites_by_threshold` now spans **0.15 / 0.5 / 1 / 2 / 3 / 5 / 10 / 20 / 50 MW** (150kW → sub-50MW); the card shows the count at the bucket nearest the developer's searched MW via `sitesForMw()`. Big small-project impact, e.g. Con Edison: ≥0.5MW = 75,755 sites vs ≥5MW = 17,705. **Audited all scrapers — NONE exclude by size:** NY filters only `community_distributed_generation='Yes'`, NJ only `metering='community solar'` (the `<2500`/`<50` checks are row-count sanity guards, not size filters; the `<25MW` is a comment on the removed ISO approach).
2. **Manual-upload ingest pipeline for the gated queues (MA, MN, CO, MD).** Can't be auto-fetched (MA = JS/Salesforce-gated; MN/CO = Xcel Salesforce portal; MD = PDF). Pattern: Aden downloads monthly → Supabase Storage → ingest parser. Not yet built.
3. **RED queue sources stay curated:** IL Illinois Shines + CA californiadgstats + PJM Data Miner 2 all forbid redistribution. Paid aggregator interconnection.fyi (16 states) is quote-only/closed-license (Aden investigating) — the only path to IL + consistent queue data. Aden's call on internal-score-only use of a no-redistribute source.
4. **Browser-verify** NY/NJ IX cards + MD hosting-capacity block render on prod (couldn't pop a browser unprompted).
...
- **County NAME-collision fix.** `allCounties.json` disambiguated 6 independent-city collisions (MD Baltimore, MO St. Louis, VA Fairfax/Franklin/Richmond/Roanoke) with a ` (city)` marker; shared `resolveCountyFips()` in `programData.js` (exact base-match, city-vs-county aware) replaced the buggy `ilike('%name%')` in the geospatial/NMTC/HUD resolvers — was loading the WRONG county's wetland/farmland data for those 6. Validated 19/19 vs live DB (`scripts/probe-county-resolve.mjs`).
- **County-geo ROW dedup (Aden's flagged 3,969-vs-3,143 dup).** Root cause: the SSURGO farmland ingest derived `county_fips` as ST+3-digit-suffix and accepted SSURGO special survey areas (suffix ≥600 — national parks/reservations/multi-county; e.g. FL608→12608) that aren't real counties → 826 spurious rows. Verified 0 real counties were missing coverage (`scripts/probe-county-geo-dups.mjs`). Fix: `_refresh-geospatial-farmland.js` now gates CONUS upserts on the canonical county-FIPS set (won't recur); migration `065` purges the existing 826 (GATED — Aden applies).

### DONE 2026-05-23 — NJ scraper + multi-source generalization (pushed; cutover gated)
- **NJ (JCP&L) distribution scraper** `api/scrapers/_refresh-nj-dg.js` — JCP&L monthly NJ interconnection queue `.xlsx` (BPU docket QO21010085), filtered to metering type = Community Solar → 33 real CS / 84 MW. JCP&L territory only (PSE&G/ACE/Rockland file as PDFs in the docket — not wired). Active-queue only → `completed_*` null. `data_source='nj_ic_queue'`.
- **Generalized the cs_pipeline layer for multiple states:** `programData.CS_PIPELINE_SOURCES` registry drives `signalType` + honest per-source UI labels (`sourceLabel/sourceRegion/sourceNote`); `completedProjects` null when a source has no energized history (UI omits it — unknown ≠ zero). IX card / §04 summary / DevFeasibility now source-driven (not hardcoded NY-Sun). Generic `scripts/apply-dist-cutover.mjs <STATE> [--apply]` replaces apply-ny-dg.
- **MA can't be auto-scraped** — DOER queue is JS/Salesforce-gated → manual-upload bucket. **Aggregator (interconnection.fyi) = quote-only** (no public price). **Hosting-capacity guide** written: `docs/hosting-capacity-sourcing.md`.
...
- **🔧 DATA-REFRESHED SIGNAL PROPAGATION (2026-05-22).** After the build-mode fix the Footer showed a date again, but it stayed stale after an admin Refresh until a hard reload. Two causes: each surface fetched the last-refresh timestamp ONCE on mount (never re-fetched), and the endpoint sent `public,max-age=60`. Fix: new shared hook `src/lib/useDataRefresh.js` (fetch `cache:'no-store'`; re-fetch on the `tractova-cache` BroadcastChannel invalidate that admin Refresh already posts + on window focus + 5-min poll; 30s tick to age labels). Footer / Dashboard / Library now consume it (removed 3 duplicated fetches). `api/data-health` last-refresh `Cache-Control` → `no-store`. Verified live: endpoint sends no-store, Footer renders "Data refreshed: 9m ago", fetch fires (200). Admin Refresh now updates the caption immediately on every open surface, no reload.
...
- `297987d` **fix: data-refreshed signal propagates across platform on admin Refresh (shared useDataRefresh hook + no-store)**
...
1. **🎨 DASHBOARD REVAMP — IN PROGRESS (started 2026-05-22).** Aden wants a *mild*, developer-first Dashboard polish, reference-driven. Refs studied: two Mobbin dashboards (Quicken — "X left of Y" progress bars; Cake — period-over-period delta panel + ranked bars) and a Godly clip (marketing-site motion → noted it belongs on the Lens result page, NOT the Dashboard tool). Key principle: motion that *informs*, not decorates (tool ≠ marketing). Plan = 3 phases, one commit each:
   - **Phase 1 — SHIPPED (`be85892`, Aden-approved "looks great"):** MetricsBar count-up numbers + loading shimmer skeletons; staggered fade-rise entrance for metrics/markets/map (Reveal helper). Reduced-motion aware.
   - **Phase 2 — SHIPPED (`dd72a47`):** map states **pulse** (breathing teal outline, new USMap Layer 3 + `.state-pulse` keyframe in index.css) when their score moved WoW (`deltaMap`); degrades to nothing when no deltas (verified render path with a temp forced set, reverted). ALSO removed the stale per-metric "⚠ data verified Nd ago" caption (used deprecated state_programs.updated_at, contradicted the canonical header pill). NOTE: currently 0 WoW deltas in prod data, so pulses are dormant until markets actually move — by design.
   - **Phase 3 — SHIPPED (`ae65768`):** state detail panel — runway-pressure bar under "Est. program runway" (fill = months/24-mo ref, urgency-colored via new `RUNWAY_BAR`; illustrative gauge, no original-total stored); WoW "▲N pts this week" chip in the panel header (dormant until deltas, like the map pulse). Dashboard passes `delta` from `deltaMap`. **NOT A BUG that it's sparse:** the runway section (badge + bar) only renders for the 5 states with `enrollment_rate_mw_per_month` seeded — **CO, IL, MA, NJ, NY** (computeRunway returns null otherwise). Aden confirmed keep-as-is (honest; don't fabricate enrollment rates). Broadening = a future data-seeding task with real sources.
   - **All 3 phases done.** Dashboard revamp complete pending Aden's prod review. Phases 2-3 change-signals (map pulse + WoW chip) are DORMANT until WoW deltas exist in data — by design.
