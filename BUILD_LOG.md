...
**Real live IX coverage = 10 CS states** (NY, NJ via queue · MD, MA, CA, CT, ME, RI, CO, MN via hosting capacity) + PA bonus. Everything else curated · zero fabricated "live".
1. **Hosting-capacity feeds — 15 utilities / 9 CS states + PA (2026-05-23).** `hosting_capacity_data` live: **CA** (SCE + PG&E), **NY** (Con Edison, National Grid, O&R, Central Hudson, NYSEG/RG&E), **MA** (National Grid), **MD** (BGE), **PA** (PECO), **CT** (Eversource), **ME** (Central Maine Power / Avangrid), **RI** (Rhode Island Energy), **CO + MN** (Xcel — STRING fields via pull+parse). Scraper handles: server-side count+buckets+max (outStatistics→orderBy fallback), unit-aware kW/MW, 60s timeout for slow self-hosted MapServers (Eversource), and a **pull+parse path** for STRING capacity fields. Metric = `sites_by_threshold` (0.15→50MW buckets, every size) shown at the bucket nearest the developer's MW; max is null-safe (CT's MapServer won't return max — UI hides it).
   ...
   - **Still-blocked CS states (no scraper possible):** IL, VA, FL, HI, MI, WI, NM, OR, WA.
