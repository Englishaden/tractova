...
### Critical Gaps Found in Deep Audit

1. **Tech type is cosmetic**: C&I, BESS, and Hybrid don't change CAPEX, IX profile, revenue model, permitting, or scenarios. Only CS gets real analysis. Non-CS gets generic placeholder text.
2. **Placeholder text visible to users**: "not yet seeded" in Search.jsx (lines 819, 849), "not yet available" in countyData.js (lines 1690, 1701). Screams unfinished product.
3. **Typography inconsistent**: MetricsBar text-[2rem] (32px), Search text-xl (20px), Library text-xl (20px), Landing text-2xl (24px) — no unified scale for metric numbers.
4. **Revenue stack colors**: Monochromatic amber (#BA7517, #D97706) used for everything. Needs purpose-mapped palette where streams are visually distinct.
5. **Site Control is thin**: Binary land/wetland flags + substation proximity. No hosting capacity, no population density, no acreage context.
6. **Profile page is MVP-basic**: Just name, email, plan status. No avatar, no activity history, no preferences.
7. **Library dark theme crowding**: Expanded cards show 20+ data elements in 2-column layout. Needs better visual grouping.
8. **Score model is arbitrary**: Weights (40/35/25) have no stated rationale. "Score" implies objective truth — should be reframed.
9. **Cross-surface flow is one-directional**: Lens → Library (save) works. Library → Lens only pre-fills state, user re-enters county, MW, stage, tech manually.
...
### AI Insights Details (for reference during implementation)

Current `api/lens-insight.js`:
- Calls Claude Sonnet with ~800-1500 token context
- System prompt positions as "$5,000 boutique consultant" — 11 explicit rules
- Output schema: `{ brief, primaryRisk, topOpportunity, immediateAction }`
- 3-tier JSON parser (strict → regex extraction → field extraction → raw text fallback)
