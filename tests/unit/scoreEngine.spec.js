...
describe('computeSubScores — main entry', () => {
  ...
  it('FEMA NRI flood is a third site constraint: a high risk rating subtracts the 12-pt penalty', () => {
    ...
    const lowFlood  = computeSubScores(base, { geospatial: { wetlandCoveragePct: 5, primeFarmlandPct: 10, floodRiskRating: 'relatively_low' } }, '', 'Community Solar')
    const highFlood = computeSubScores(base, { geospatial: { wetlandCoveragePct: 5, primeFarmlandPct: 10, floodRiskRating: 'very_high' } }, '', 'Community Solar')
    expect(noFlood.site).toBe(82)    // no rating → no penalty
    expect(lowFlood.site).toBe(82)   // relatively_low → not constrained → no penalty
    expect(highFlood.site).toBe(70)  // very_high → 82 − 12
  })
  ...
  it('flood penalty never drives the site sub-score below 0', () => {
    ...
    const r = computeSubScores(
      { id: 'IA', csStatus: 'pending', ixDifficulty: 'moderate' },
      { geospatial: { wetlandCoveragePct: 50, primeFarmlandPct: 80, floodRiskRating: 'relatively_high' } }, '', 'Community Solar',
    )
